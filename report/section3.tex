...
\textbf{Two model families.} (1) \emph{Classical baseline}: We engineered forensic
features, which are a 32-bin radial FFT spectrum, colour/saturation statistics and
high-pass residual statistics. These features were fed to a histogram gradient-boosted tree. It
...
training loop to \emph{measure} its own step time and sizes a one-cycle LR
...
the $0.8$ recall target while keeping the FPR safely below the $0.20$ limit, and shows a strong improvement over the classical baseline (recall~\BaseValRecall{}). We made a few key choices that mattered. The first one was squaring the inputs byremoving the
aspect-ratio shortcut. Without it validation FPR collapses. The second one was BatchNorm (fast
