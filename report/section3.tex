\section{Modelling and Tuning under Time Constraints}
\textbf{Protocol}\\
 We maximise recall on AI images subject to a false-positive
...
$\tau=\Targetfpr$. This leaves a safe margin below the $0.20$ limit for the hidden holdout. The threshold is never hand-set, and \texttt{validation/} is held out purely to
verify the realised FPR.

\textbf{Two model families}\\
 (1) \emph{Classical baseline}: We engineered forensic
features (a 32-bin radial FFT spectrum, colour/saturation statistics and
high-pass residual statistics) and fed them to a histogram gradient-boosted tree. It
...
(Fig.~\ref{fig:imp}). (2) \emph{CNN from scratch}: We designed a compact VGG-style network
...
\textbf{Efficient, reproducible training}\\
 The Appendix~C reference takes
$\RefSeconds$\,s for 35 steps on our machine and our \texttt{train.py} runs in
$\TrainSeconds$\,s ($\TrainRatio\times$ reference, within the $5\times$
guideline) and well under the 1800\,s timeout. Because CPU speed varies, we adjusted the
training loop to \emph{measure} its own step time and size a one-cycle LR
...
\textbf{Results and ablations}\\
 Table~\ref{tab:t2} compares both families. The
CNN reaches recall~$\CnnValRecall$ at FPR~$\CnnValFpr$ on validation, meeting
the $0.8$ recall target while keeping the FPR safely below the $0.20$ limit, and shows a strong improvement over the classical baseline (recall~\BaseValRecall{}). We made a few key choices that mattered. The first one was squaring the inputs, removing the
aspect-ratio shortcut; without it, the validation FPR explodes. The second one was BatchNorm (fast
from-scratch convergence in the few affordable epochs). The third one was the stride-2 stem, which lets
us keep $\ImgSize$px inputs within budget. The fourth choice was calibrating on a held-out split
...
\texttt{validation\_augmented} ($\BaseAugFpr$/$\CnnAugFpr$), which is exactly
what the augmented model repairs ($\AugAugFpr$).}
\label{tab:t2}
\end{table}
